<?php
$permittedFormats = ['txt', 'html', 'css', 'js'];
...
<div class="site">
    ...
    <ul>
        ...
        <li>
            ...
            <a href="/admin/php/delete.php?dir=<?= $dir; ?>&remove=<?= $path;?>&type=dir" class="button del">Удалить</a>
            <a href="/admin/php/rename.php?dir=<?= $dir; ?>&rename=<?= $path;?>" class="button rename">Переименовать</a>
        ...
        <li>
            <?= $path; ?>
            <a href="/admin/php/delete.php?dir=<?= $dir; ?>&remove=<?= $path;?>&type=file" class="button del">Удалить</a>
            <a href="/admin/php/rename.php?dir=<?= $dir; ?>&rename=<?= $path;?>" class="button rename">Переименовать</a>
            <?php $path = $dir . '\\'.$path;
            $pathinfo = pathinfo($path);
            $extension = $pathinfo['extension'] ?? 'none';
            if (preg_match('/txt|css|html|js/ui', $extension)) :?>
            <a href="/admin/php/edit.php?dir=<?= $dir; ?>&file=<?= $path; ?>"  class="button edit">Редактировать</a>
            <?php endif; ?>
        </li>
    ...
    <div class="forms">
        <form action="/admin/php/create.php?dir=<?= $dir; ?>&type=dir" method="post">
        ...
        <form action="/admin/php/create.php?dir=<?= $dir; ?>&type=file" method="post">
            <label>Имя нового файла<input type="text" name="newFile" placeholder="Имя нового файла"></label>
            <label>Расширение:
                <select name="extension">
                    <?php foreach ($permittedFormats as $format) :?>
                    <option value="<?= $format; ?>" <?= ($format == "txt") ? "selected" : ""; ?>><?= $format; ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
        ...
        <form enctype="multipart/form-data" action="/admin/php/upload.php?dir=<?= $dir; ?>" method="POST">
